...
#task_11  ##completed
    ...
    for i in range(len(a)):
        str1=''.join(a[i]['genre'])    
        if str1 not in genre_list: 
            genre_list.append (str1) 
    genre_dict={}
    for ty_pe in genre_list:
        genre_dict[ty_pe]=0
    for ty_pe in genre_list:
        for movie_type in a:
            if ty_pe in movie_type['genre']:
                genre_dict[ty_pe]+=1
    print (genre_dict)
...
for i in list1:
    url_list1.append(i["url"][27:39].strip("/"))
for i in url_list1:
    a=requests.get('https://www.imdb.com/title/'+i+'/fullcredits?ref_=tt_cl_sm#cast')
    b=BeautifulSoup(a.text,'html.parser')
    print (b.find('title').get_text())
    (actors(b))
